...
import static wng.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static wng.http.JSONResponses.NO_PASSWORD_IN_CONFIG;

public final class API {

    public static final int TESTNET_API_PORT = 6826;
    public static final int TESTNET_API_SSLPORT = 6827;
    ...
    public static final String adminPassword = Wng.getStringProperty("wng.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = Wng.getIntProperty("wng.maxAPIRecords");
    static final boolean enableAPIUPnP = Wng.getBooleanProperty("wng.enableAPIUPnP");
    ...
    static {
        List<String> disabled = Wng.getStringListProperty("wng.disabledAPIs");
        Collections.sort(disabled);
        disabledAPIs = Collections.unmodifiableList(disabled);
        disabled = Wng.getStringListProperty("wng.disabledAPITags");
        Collections.sort(disabled);
        ...
        List<String> allowedBotHostsList = Wng.getStringListProperty("wng.allowedBotHosts");
        ...
        boolean enableAPIServer = Wng.getBooleanProperty("wng.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : Wng.getIntProperty("wng.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : Wng.getIntProperty("wng.apiServerSSLPort");
            final String host = Wng.getStringProperty("wng.apiServerHost");
            disableAdminPassword = Wng.getBooleanProperty("wng.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = Wng.getBooleanProperty("wng.apiSSL");
            //
            ...
            if (!enableSSL || port != sslPort) {
                ...
                connector.setIdleTimeout(Wng.getIntProperty("wng.apiServerIdleTimeout"));
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(Wng.getUserHomeDir()).resolve(Paths.get(Wng.getStringProperty("wng.keyStorePath"))).toString();
                Logger.logInfoMessage("Using keystore: " + keyStorePath);
                sslContextFactory.setKeyStorePath(keyStorePath);
                sslContextFactory.setKeyStorePassword(Wng.getStringProperty("wng.keyStorePassword", null, true));
                ...
                List<String> ciphers = Wng.getStringListProperty("wng.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = Wng.getStringProperty("wng.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{Wng.getStringProperty("wng.apiWelcomeFile")});
            }

            String javadocResourceBase = Wng.getStringProperty("wng.javadocResourceBase");
            ...
            ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/wng");
            servletHolder.getRegistration().setMultipartConfig(new MultipartConfigElement(
                    null, Math.max(Wng.getIntProperty("wng.maxUploadFileSize"), Constants.MAX_TAGGED_DATA_DATA_LENGTH), -1L, 0));

            GzipHandler gzipHandler = new GzipHandler();
            if (!Wng.getBooleanProperty("wng.enableAPIServerGZIPFilter")) {
                gzipHandler.setExcludedPaths("/wng");
            }
            ...
            if (Wng.getBooleanProperty("wng.apiServerCORS")) {
                FilterHolder filterHolder = apiHandler.addFilter(CrossOriginFilter.class, "/*", null);
                filterHolder.setInitParameter("allowedHeaders", "*");
                filterHolder.setAsyncSupported(true);
            }

            if (Wng.getBooleanProperty("wng.apiFrameOptionsSameOrigin")) {
